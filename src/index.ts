...
server.tool(
  'a11y_lookup_wcag',
  'Look up WCAG criteria by number, keyword, or level. Returns criterion details including description, techniques, failures, and test suggestions.',
  lookupWcagInputSchema,
  async (args) => {
...
server.tool(
  'a11y_check_pattern',
  'Analyze HTML code snippet for common accessibility issues. Returns list of issues with WCAG references and fix suggestions.',
...
server.tool(
  'a11y_suggest_fix',
  'Get remediation patterns for accessibility issues. Includes before/after code examples and related patterns.',
...
server.tool(
  ...
  'Generate accessibility documentation for UI components. Includes keyboard interactions, ARIA attributes, and screen reader expectations.',
  documentComponentInputSchema,
  async (args) => {
...
server.tool(
  'a11y_audit_summary',
  'Generate an accessibility audit report from a list of issues. Includes executive summary, severity analysis, and remediation priorities.',
