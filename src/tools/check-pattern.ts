...
function checkForIssues(html: string, context?: string): AccessibilityIssue[] {
...
function formatIssues(issues: AccessibilityIssue[], context?: string): string {
  if (issues.length === 0) {
...
export async function checkPattern(args: { html: string; context?: string }): Promise<string> {
