import { z } from 'zod';
import { findByNumber } from '../services/wcag-service.js';
...
function getWcagDetails(criterionId) {
...
function categorizeByWcagLevel(issues) {
    const categories = {
        A: [],
        AA: [],
        AAA: [],
    ...
    issues.forEach(issue => {
        ...
        issue.wcagCriteria.forEach(id => {
            ...
            if (details) {
                if (details.level === 'AAA')
                    highestLevel = 'AAA';
                else if (details.level === 'AA' && highestLevel !== 'AAA')
                    highestLevel = 'AA';
            }
        });
...
function getTotalCount(issues) {
    return issues.reduce((total, issue) => total + (issue.count || 1), 0);
}
function generateExecutiveSummary(issues, bySeverity, byLevel) {
...
function formatIssueList(issues, title) {
    if (issues.length === 0)
        return '';
    const lines = [
        `### ${title} (${issues.length})`,
...
function generateDetailedFindings(bySeverity, byLevel) {
    const sections = [];
    // By Severity
...
function generateRemedationPriorities(issues) {
    const lines = [
